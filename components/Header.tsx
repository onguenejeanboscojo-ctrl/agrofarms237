    ...
    <header className="fixed inset-x-0 top-0 z-[100] border-b border-ink/10 bg-bg/90 backdrop-blur-md">
      <div className="mx-auto flex h-[68px] max-w-[1180px] items-center justify-between px-5">
        ...
        <nav className="hidden md:flex items-center">
          {LINKS.map((l) => (
            <Link key={l.href} href={l.href} className="ml-7 text-[14.5px] font-semibold text-inkSoft hover:text-ink">
              {l.label}
            </Link>
          ))}
          <Link href="/commander" className="btn btn-gold ml-7">Commander</Link>
        </nav>
        <button
        ...
        >
          <span className="block h-[2px] w-[18px] bg-ink relative before:content-[''] before:absolute before:-top-[6px] before:h-[2px] before:w-[18px] before:bg-ink after:content-[''] after:absolute after:top-[6px] after:h-[2px] after:w-[18px] after:bg-ink" />
        </button>
      </div>
      {open && (
        <div className="md:hidden bg-paper px-5 pb-6">
          ...
          <Link href="/commander" onClick={() => setOpen(false)} className="btn btn-gold mt-5 w-full justify-center">
            Commander
          </Link>
        </div>
      )}
